const config = {
    ...
    physics: {
        default: 'arcade',
        arcade: { gravity: { y: 800 }, debug: false } // Cambia a true si quieres ver colisiones
    },
    scene: { preload, create, update },
    backgroundColor: '#000000' // Fondo negro del juego
};
...
function spawnCoins(scene) {
    for (let i = 0; i < 20; i++) {
        let x = i * PLATFORM_WIDTH + Phaser.Math.Between(50, 150);
...
function spawnEnemies(scene) {
    for (let i = 0; i < 5; i++) {
        ...
        let enemy = enemies.create(x, 402, 'enemy').setBounce(1).setImmovable(true);
